...
export default async function AgendaPage() {
  ...
  const upcomingVisits = visitas.filter(
    (v) => v.status === "agendada" && v.agendadaPara >= agora
  );

  const allVisitsSorted = [...visitas].sort(
    (a, b) => b.agendadaPara.getTime() - a.agendadaPara.getTime()
  );
        ...
        <div className="lg:col-span-2 space-y-4">
          {upcomingVisits.length === 0 ? (
            ...
            upcomingVisits.map((v) => (
              <div key={v.id} className="bg-white border border-gray-100 p-5 flex gap-4">
                <div className="w-14 h-14 bg-[var(--brand-yellow)] flex flex-col items-center justify-center shrink-0">
                  <span className="font-black text-xl text-[var(--brand-dark)] leading-none">
                    {v.agendadaPara.getDate()}
                  </span>
                  <span className="text-[9px] font-bold uppercase text-[var(--brand-dark)]">
                    {v.agendadaPara.toLocaleString("pt-BR", { month: "short" })}
                  </span>
                </div>
                <div className="flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="font-bold text-[var(--brand-dark)]">{v.clienteNome}</p>
                      <p className="text-gray-400 text-sm">{v.imovel?.titulo ?? "Imóvel não informado"}</p>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-3 mt-2 text-xs text-gray-400">
                    <span className="flex items-center gap-1">
                      <Clock size={11} />
                      {v.agendadaPara.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}
                    </span>
                    {v.corretor && (
                      <span className="flex items-center gap-1">
                        <User size={11} />
                  ...
                  <div className="flex gap-2 mt-3">
                    {v.lead && (
                      <Link href={`/admin/leads/${v.lead.id}`} className="text-xs bg-[var(--brand-dark)] hover:bg-[var(--brand-dark-secondary)] text-[var(--brand-yellow)] font-bold uppercase tracking-wider px-3 py-1.5 transition-colors">
                        Ver Lead
                      </Link>
                    )}
                    <form action={atualizarStatusVisita.bind(null, v.id, "realizada")}>
                      <button type="submit" className="text-xs bg-green-50 hover:bg-green-100 text-green-600 font-bold uppercase tracking-wider px-3 py-1.5 transition-colors flex items-center gap-1">
                        <CheckCircle2 size={11} /> Realizada
                      </button>
                    </form>
                    <form action={atualizarStatusVisita.bind(null, v.id, "cancelada")}>
                      <button type="submit" className="text-xs bg-red-50 hover:bg-red-100 text-red-500 font-bold uppercase tracking-wider px-3 py-1.5 transition-colors flex items-center gap-1">
                ...
                <div key={v.id} className="px-5 py-3 flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-gray-400 text-xs">
                      {v.imovel?.titulo ?? "Imóvel não informado"} · {v.agendadaPara.toLocaleString("pt-BR", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                    </p>
                  </div>
          ...
          <div className="bg-white border border-gray-100 p-5">
            ...
            <form action={criarVisita} className="space-y-2">
              <input
                ...
                required
                className="w-full border border-gray-200 px-3 py-2 text-xs focus:outline-none focus:border-[var(--brand-yellow)] bg-gray-50"
              />
              <input
                name="clienteTel"
                type="tel"
              ...
              <button
                type="submit"
                className="w-full bg-[var(--brand-yellow)] hover:bg-[var(--brand-yellow-dark)] text-[var(--brand-dark)] font-bold text-xs uppercase tracking-wider py-2 transition-colors"
              >
                Agendar Visita
              </button>
            </form>
