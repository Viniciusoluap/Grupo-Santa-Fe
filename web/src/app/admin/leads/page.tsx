    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <div className="flex border border-gray-200">
            <button onClick={() => setView("kanban")} className={`px-3 py-2 text-xs font-bold uppercase transition-colors ${view === "kanban" ? "bg-[var(--brand-dark)] text-[var(--brand-yellow)]" : "text-gray-400 hover:bg-gray-50"}`}>
              <LayoutGrid size={14} />
            </button>
            <button onClick={() => setView("list")} className={`px-3 py-2 text-xs font-bold uppercase transition-colors ${view === "list" ? "bg-[var(--brand-dark)] text-[var(--brand-yellow)]" : "text-gray-400 hover:bg-gray-50"}`}>
              <List size={14} />
            </button>
          </div>
          <Link href="/admin/leads/novo" className="flex items-center gap-1.5 bg-[var(--brand-yellow)] hover:bg-[var(--brand-yellow-dark)] text-[var(--brand-dark)] font-bold text-xs uppercase tracking-wider px-4 py-2.5 transition-colors">
      ...
      <div className="grid grid-cols-3 md:grid-cols-7 gap-2">
        {(Object.entries(LEAD_STATUS_CONFIG) as [LeadStatus, typeof LEAD_STATUS_CONFIG[LeadStatus]][])
          ...
          .map(([status, cfg]) => (
            <div key={status} className="bg-white border border-gray-100 p-3 text-center hover:border-[var(--brand-yellow)] transition-colors cursor-pointer">
              <p className="font-black text-[var(--brand-dark)] text-2xl">{statusCounts[status] ?? 0}</p>
              <span className={`text-[9px] font-bold px-1.5 py-0.5 uppercase inline-block mt-1 ${cfg.bgColor} ${cfg.color}`}>{cfg.label}</span>
            </div>
          ))}
      </div>
                ...
                {filtered.map((lead) => {
                  ...
                  return (
                    <tr key={lead.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 hidden md:table-cell text-sm text-gray-500">{lead.service}</td>
                      <td className="px-4 py-3 hidden lg:table-cell text-sm text-gray-500">{lead.assignedTo}</td>
                      <td className="px-4 py-3 hidden lg:table-cell text-right font-bold text-sm">{lead.budget ? formatCurrency(lead.budget) : "—"}</td>
                      <td className="px-4 py-3 text-center">
                        <span className={`text-[10px] font-bold px-2 py-0.5 uppercase ${cfg.bgColor} ${cfg.color}`}>{cfg.label}</span>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <Link href={`/admin/leads/${lead.id}`} className="text-xs font-bold text-[var(--brand-yellow)] hover:underline">Ver</Link>
                      </td>
                    </tr>
                  );
                })}
