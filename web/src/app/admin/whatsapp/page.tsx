...
const MESSAGE_LOGS: MessageLog[] = [
  { id: "ml1", to: "Carlos Mendes", phone: "(62) 9 8888-1111", template: "Boas-vindas ao lead", sentAt: "2026-06-04T09:15:00", status: "lida" },
  { id: "ml2", to: "Beatriz Santos", phone: "(62) 9 7777-2222", template: "Confirmação de visita", sentAt: "2026-06-04T09:30:00", status: "entregue" },
  { id: "ml3", to: "Roberto Alves", phone: "(62) 9 6666-3333", template: "Atualização do financiamento", sentAt: "2026-06-04T10:00:00", status: "enviada" },
  { id: "ml4", to: "Ana Paula Lima", phone: "(62) 9 5555-4444", template: "Lembrete de documentos", sentAt: "2026-06-03T14:22:00", status: "lida" },
  { id: "ml5", to: "Marcos Ferreira", phone: "(62) 9 8877-5566", template: "Follow-up após 48h", sentAt: "2026-06-03T11:00:00", status: "falhou" },
  { id: "ml6", to: "Rodrigo Mendes", phone: "(62) 9 7766-4455", template: "Envio de proposta", sentAt: "2026-06-02T16:45:00", status: "lida" },
];
...
export default function WhatsAppPage() {
  ...
  const lidas = MESSAGE_LOGS.filter((m) => m.status === "lida").length;
  const entregues = MESSAGE_LOGS.filter((m) => m.status === "entregue").length;
  const falhas = MESSAGE_LOGS.filter((m) => m.status === "falhou").length;
  const taxa = Math.round(((lidas + entregues) / MESSAGE_LOGS.length) * 100);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Mensagens enviadas", value: MESSAGE_LOGS.length, icon: Send, color: "text-[var(--brand-yellow)]" },
                ...
                return (
                  <label key={l.id} className={`flex items-center gap-3 p-2.5 cursor-pointer border transition-colors ${selected ? "border-[var(--brand-yellow)] bg-[var(--brand-yellow)]/5" : "border-transparent hover:bg-gray-50"}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-[var(--brand-dark)] truncate">{l.name}</p>
                      <p className="text-xs text-gray-400 flex items-center gap-1"><Phone size={10} /> {l.phone}</p>
                    </div>
                  </label>
                );
              })}
      ...
      {activeTab === "templates" && (
        <div className="space-y-4">
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setCategoryFilter("todas")} className={`px-3 py-1.5 text-xs font-bold uppercase transition-colors ${categoryFilter === "todas" ? "bg-[var(--brand-dark)] text-[var(--brand-yellow)]" : "border border-gray-200 text-gray-400 hover:bg-gray-50"}`}>
            ...
            {(Object.entries(CATEGORY_LABELS) as [TemplateCategory, string][]).map(([cat, label]) => {
              ...
              return (
                <button key={cat} onClick={() => setCategoryFilter(cat)} className={`px-3 py-1.5 text-xs font-bold uppercase transition-colors ${categoryFilter === cat ? "bg-[var(--brand-dark)] text-[var(--brand-yellow)]" : "border border-gray-200 text-gray-400 hover:bg-gray-50"}`}>
                  {label} ({count})
                </button>
              );
            })}
