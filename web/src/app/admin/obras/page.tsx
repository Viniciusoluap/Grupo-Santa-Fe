...
export default function ObrasPage() {
  ...
  const totalValue = obras.reduce((s, o) => s + o.totalValue, 0);
  const totalPaid = obras.reduce((s, o) => s + o.paidValue, 0);
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        {filtered.map((o) => {
          ...
          const saldo = o.totalValue - o.paidValue;
          return (
            <div key={o.id} className="bg-white border border-gray-100 p-5">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <h3 className="font-bold text-[var(--brand-dark)]">{o.name}</h3>
                    <span className={`text-[10px] font-bold px-2 py-0.5 uppercase ${cfg.bgColor} ${cfg.color}`}>{cfg.label}</span>
                  </div>
                  <p className="text-gray-400 text-sm">{o.clientName} · {o.address}</p>
                  <p className="text-gray-400 text-xs mt-0.5">Resp.: {o.responsibleEngineer} · {o.area} m²</p>
                </div>
                <div className="text-right shrink-0">
                  <p className="font-black text-[var(--brand-dark)] text-lg">{formatCurrency(o.totalValue)}</p>
              ...
              <div className="mt-4 grid grid-cols-3 gap-4">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-2 bg-gray-100 overflow-hidden">
                      <div className={`h-full ${o.progress === 100 ? "bg-green-500" : "bg-[var(--brand-yellow)]"}`} style={{ width: `${o.progress}%` }} />
                    </div>
                    <span className="text-xs font-bold text-[var(--brand-dark)]">{o.progress}%</span>
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-[var(--brand-dark)]">{o.diario.length} registros</p>
                </div>
              </div>
