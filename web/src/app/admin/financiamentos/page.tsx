...
import { financiamentos, getChecklistProgress } from "@/lib/data/financiamentos";
...
export default function FinanciamentosPage() {
  ...
  const filtered = financiamentos.filter((f) => {
    const matchSearch = !search ||
      ...
      f.corretor.toLowerCase().includes(search.toLowerCase()) ||
      (f.protocolNumber?.toLowerCase().includes(search.toLowerCase()) ?? false);
    const matchStatus = statusFilter === "todos" || f.status === statusFilter;
    return matchSearch && matchStatus;
  });

  const totalFinanciado = financiamentos.reduce((s, f) => s + f.financingValue, 0);
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "Taxa média", value: `${(financiamentos.reduce((s, f) => s + f.rate, 0) / financiamentos.length).toFixed(1)}% a.a.`, sub: "taxa de juros" },
        ].map(({ label, value, sub }) => (
                ...
                return (
                  <tr key={f.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <p className="font-medium text-[var(--brand-dark)]">{f.clientName}</p>
                      <p className="text-xs text-gray-400">{f.corretor}</p>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                      <p className="text-sm text-gray-700 max-w-[200px] truncate">{f.property}</p>
                      {f.protocolNumber && <p className="text-xs text-gray-400">{f.protocolNumber}</p>}
                    </td>
                    <td className="px-4 py-3 hidden lg:table-cell">
                      <p className="text-xs font-medium text-[var(--brand-dark)]">{TIPO_CONFIG[f.tipo].label}</p>
                      <p className="text-xs text-gray-400">{BANCO_CONFIG[f.banco].label}</p>
                    </td>
                    <td className="px-4 py-3 text-right hidden lg:table-cell">
                      <p className="font-bold text-sm">{formatCurrency(f.financingValue)}</p>
                      <p className="text-xs text-gray-400">{f.rate}% a.a.</p>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <div className="w-16 h-1.5 bg-gray-100 overflow-hidden">
                          <div className={`h-full ${progress.pct === 100 ? "bg-green-500" : "bg-[var(--brand-yellow)]"}`} style={{ width: `${progress.pct}%` }} />
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-center">
